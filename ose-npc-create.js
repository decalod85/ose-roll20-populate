...
const createNPCs = (monster) => {
	...
	character.set("gmnotes", monster.description + "<br>" + monster.notes.join("<br>"));
...
on("chat:message", function(msg) {
    if (msg.type === "api" && msg.content.startsWith("!ose-npc-create")) {
		
		let args = msg.content.split(" ").slice(1).join(" "); // Extract monster name
        
        if (!args) {
            sendChat("API", "Please provide a monster name.");
            return;
        }
        
        let monster = monsters.find(m => m.name.toLowerCase() === args.toLowerCase());
        
        if (!monster) {
            sendChat("API", `Monster '${args}' not found.`);
            return;
        }
        
        createNPCs(monster); // Function that handles monster creation
    }
});
